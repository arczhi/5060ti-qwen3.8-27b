...
- Context: start at `98304` tokens; test lower before attempting 128K
- Parallelism: `1`
...
2. Set `MODEL_DIR`, `MODEL_FILE`, `DRAFT_FILE`, `LLAMA_IMAGE`, `HOST_PORT`, and `CONTEXT` in the environment. The startup script has portable defaults, but deployment paths should be explicit.
...
- If the server reports a prefill memory guard error, reduce context or free VRAM before raising memory limits. On a 16GB card, a 96K configuration is already an aggressive starting point.
